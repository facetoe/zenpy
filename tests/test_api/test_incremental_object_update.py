...
class TestIncrementalObjectUpdate(ZenpyApiTestCase):
    ...
    def setUp(self):
        with self.recorder.use_cassette(
            ...
            self.ticket = self.zenpy_client.tickets(id=1) # TESTING_CHANGE OK TO KEEP
            self.ticket._clean_dirty()

    def test_new_ticket_only_serializes_id(self):
        self.assertEqual(dict(id=self.ticket.id), self.ticket.to_dict(serialize=True))

    def test_modified_attribute_serialized(self):
        self.ticket.subject = "YOLOTIME"
        self.assertEqual(
            dict(id=self.ticket.id, subject=self.ticket.subject),
    ...
    def test_clean_dirty(self):
        ...
        self.assertEqual(
            self.ticket.to_dict(serialize=True).keys(), self.ticket.to_dict().keys()
        )
