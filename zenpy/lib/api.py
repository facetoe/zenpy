...
class BaseApi(object):
    ...
    def _post_optional(self, url, **kwargs):
        """Sends a post request without any forced headers.
        Useful for ie. multipart formpost requests, as it leaves
        the handling of headers to requests."""
...
class MacroApi(CRUDApi):
    ...
    def upload_attachment(self, fp, api_object="unassociated_macro", target_name=None):
        """
        Upload a file to Zendesk.
        :param fp: file object, StringIO instance, content, or file path to be
                   uploaded
        ...
        if api_object:
            return MacroAttachmentUploadRequest(self).post(
                fp, target_name=target_name, api_object=api_object)

        else:
            return MacroAttachmentUploadRequest(self).post(
                fp, target_name=target_name, api_object="unassociated_macro")
